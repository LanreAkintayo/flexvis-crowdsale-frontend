...
export default function PresaleDetails({presaleInfo}) {
    ...
    const formattedCap = dollarUSLocale.format(fromWei(presaleInfo?.cap).toString());
    const formattedAmountRaised = dollarUSLocale.format(fromWei(presaleInfo?.weiRaised).toString());
  ...
  return (
    <div>
      ...
      <div className="w-96 flex py-2 justify-between">
        ...
        <p className="text-base">1 FLEXVIS = {1 / presaleInfo.rate} BNB</p>
      </div>
      </div>
      </div>
